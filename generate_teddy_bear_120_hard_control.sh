...
readonly OUTPUT_ROOT="${OUTPUT_ROOT:-${SCRIPT_DIR}/output/teddy_bear_hard_control_fixed_model}"
readonly OUTPUT_ROOT_IN_CONTAINER="${OUTPUT_ROOT_IN_CONTAINER:-/output/teddy_bear_hard_control_fixed_model}"
readonly FIXED_MODEL_DIR="${FIXED_MODEL_DIR:-${SCRIPT_DIR}/output/teddy_bear_fixed_spring_mass_model}"
readonly FIXED_MODEL_DIR_IN_CONTAINER="${FIXED_MODEL_DIR_IN_CONTAINER:-/output/teddy_bear_fixed_spring_mass_model}"
readonly FIXED_MODEL_DATA="${FIXED_MODEL_DATA:-${FIXED_MODEL_DIR}/simulation_data.npz}"
readonly FIXED_MODEL_DATA_IN_CONTAINER="${FIXED_MODEL_DATA_IN_CONTAINER:-${FIXED_MODEL_DIR_IN_CONTAINER}/simulation_data.npz}"
...
mkdir -p "${OUTPUT_ROOT}" "${TOPDOWN_DIR}" "${FIXED_MODEL_DIR}"
...
if [[ ! -f "${FIXED_MODEL_DATA}" ]]; then
  fixed_model_trajectory="${FIXED_MODEL_DIR}/all_topdown_control_points_stationary.npz"
  fixed_model_trajectory_in_container="${FIXED_MODEL_DIR_IN_CONTAINER}/all_topdown_control_points_stationary.npz"

  echo "Create fixed spring-mass model trajectory: ${fixed_model_trajectory}"
  docker_kubric \
    python3 examples/create_topdown_all_control_trajectory.py \
      --topdown_control_points="${TOPDOWN_CONTROL_POINTS_IN_CONTAINER}" \
      --output="${fixed_model_trajectory_in_container}" \
      --frame_start=0 \
      --frame_end=1 \
      --frame_rate=24

  echo "Create fixed spring-mass model: ${FIXED_MODEL_DATA}"
  docker_kubric \
    python3 examples/gso_spring_mass_control.py \
      ...
      --frame_rate=24 \
      --frame_end=1 \
      --job-dir="${FIXED_MODEL_DIR_IN_CONTAINER}" \
      --scratch_dir=/tmp/kubric_teddy_bear_fixed_spring_mass_model \
      --device=cuda \
      --control_trajectory="${fixed_model_trajectory_in_container}" \
      --control_physics_mode=hard \
      --camera_distance=2.4 \
      --camera_height=1.4 \
  ...
  echo "Use existing fixed spring-mass model: ${FIXED_MODEL_DATA}"
fi
...
for point_a in $(seq 0 "$((NUM_CONTROL_POINTS - 2))"); do
  for point_b in $(seq "$((point_a + 1))" "$((NUM_CONTROL_POINTS - 1))"); do
    ...
    echo "Render hard-control case: ${pair_label}"
    docker_kubric \
      python3 examples/gso_spring_mass_control.py \
        --seed="${pair_count}" \
        --asset_id="${ASSET_ID}" \
        ...
        --scratch_dir="/tmp/kubric_teddy_bear_hard_${pair_label}" \
        --device=cuda \
        --load_spring_mass_model="${FIXED_MODEL_DATA_IN_CONTAINER}" \
