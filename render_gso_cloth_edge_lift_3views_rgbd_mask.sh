...
readonly DATASET_NAME="${DATASET_NAME:-cloth_grid_pair_lift_3views_rgbd_mask}"
...
readonly FRAME_END="${FRAME_END:-120}"
...
has_complete_case() {
  ...
  for view_dir in camera_00 camera_01 camera_02; do
    for layer in image depth segmentation; do
      ...
      if [[ ! -d "${case_dir}/${view_dir}/${layer}" ]]; then
        return 1
      fi
      count=$(find "${case_dir}/${view_dir}/${layer}" \
        -maxdepth 1 \
        -type f \
        -name "${pattern}" \
        | wc -l)
...
echo "Rendered ${rendered} grid-pair cloth cases successfully."
